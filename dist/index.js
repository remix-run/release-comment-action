...
const patch = __nccwpck_require__(268)
...
async function main() {
    ...
    if (DIRECTORY_TO_CHECK)
        gitCommitArgs.push(DIRECTORY_TO_CHECK);
    ...
    if (DIRECTORY_TO_CHECK) {
        debug(`found ${prs.length} merged PRs that changed ${DIRECTORY_TO_CHECK}/*`);
    }
    else {
...
function isPullRequestResult(pr) {
...
function isReferencedResult(result) {
    ...
    let isClosingIssuesReferences = (closingIssuesReferences) => {
        return (typeof closingIssuesReferences === "object" &&
...
async function getIssuesLinkedToPullRequest(prHtmlUrl) {
  ...
  `;
    ...
    return parsed.data.resource.closingIssuesReferences.nodes.map((node) => node.number);
}
main().then(() => {
    process.exit(0);
}, (error) => {
