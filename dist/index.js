...
const bufferStream = __nccwpck_require__(5162);
...
const parseOptions = __nccwpck_require__(6063)
const { safeRe: re, t } = __nccwpck_require__(4386)
const cmp = __nccwpck_require__(1163)
const debug = __nccwpck_require__(8685)
const SemVer = __nccwpck_require__(2466)
const Range = __nccwpck_require__(3860)
...
const parseOptions = __nccwpck_require__(6063)
const Comparator = __nccwpck_require__(5221)
const debug = __nccwpck_require__(8685)
const SemVer = __nccwpck_require__(2466)
const {
  safeRe: re,
...
const { FLAG_INCLUDE_PRERELEASE, FLAG_LOOSE } = __nccwpck_require__(1166)
...
const debug = __nccwpck_require__(8685)
const { MAX_LENGTH, MAX_SAFE_INTEGER } = __nccwpck_require__(1166)
const { safeRe: re, t } = __nccwpck_require__(4386)

const parseOptions = __nccwpck_require__(6063)
const { compareIdentifiers } = __nccwpck_require__(4636)
class SemVer {
...
const parse = __nccwpck_require__(3233)
...
const eq = __nccwpck_require__(331)
const neq = __nccwpck_require__(2564)
const gt = __nccwpck_require__(2125)
const gte = __nccwpck_require__(3826)
const lt = __nccwpck_require__(6267)
const lte = __nccwpck_require__(4837)
...
const compare = __nccwpck_require__(1355)
...
const compareBuild = __nccwpck_require__(3105)
...
const internalRe = __nccwpck_require__(4386)
const constants = __nccwpck_require__(1166)
const SemVer = __nccwpck_require__(2466)
const identifiers = __nccwpck_require__(4636)
const parse = __nccwpck_require__(3233)
const valid = __nccwpck_require__(3147)
const clean = __nccwpck_require__(9071)
const inc = __nccwpck_require__(4850)
const diff = __nccwpck_require__(1748)
const major = __nccwpck_require__(133)
const minor = __nccwpck_require__(8334)
const patch = __nccwpck_require__(6187)
const prerelease = __nccwpck_require__(2007)
const compare = __nccwpck_require__(1355)
const rcompare = __nccwpck_require__(3236)
const compareLoose = __nccwpck_require__(2830)
const compareBuild = __nccwpck_require__(3105)
const sort = __nccwpck_require__(2304)
const rsort = __nccwpck_require__(5912)
const gt = __nccwpck_require__(2125)
const lt = __nccwpck_require__(6267)
const eq = __nccwpck_require__(331)
const neq = __nccwpck_require__(2564)
...
const lte = __nccwpck_require__(4837)
const cmp = __nccwpck_require__(1163)
const coerce = __nccwpck_require__(4747)
const Comparator = __nccwpck_require__(5221)
const Range = __nccwpck_require__(3860)
const satisfies = __nccwpck_require__(5651)
const toComparators = __nccwpck_require__(4904)
const maxSatisfying = __nccwpck_require__(9966)
const minSatisfying = __nccwpck_require__(9603)
const minVersion = __nccwpck_require__(5071)
const validRange = __nccwpck_require__(9365)
const outside = __nccwpck_require__(6780)
const gtr = __nccwpck_require__(637)
const ltr = __nccwpck_require__(464)
const intersects = __nccwpck_require__(9321)
const simplifyRange = __nccwpck_require__(2687)
const subset = __nccwpck_require__(6287)
module.exports = {
  parse,
  valid,
  clean,
...
const { MAX_SAFE_COMPONENT_LENGTH, MAX_SAFE_BUILD_LENGTH } = __nccwpck_require__(1166)
const debug = __nccwpck_require__(8685)
...
if (!processOk(process)) {
  ...
  var signals = __nccwpck_require__(3789)
...
const semver_1 = __importDefault(__nccwpck_require__(1228));
...
async function main() {
    ...
    debug(`> found ${prs.length} merged PR${plural} that changed ${DIRECTORY_TO_CHECK}`);
...
async function findBoundingTags() {
    ...
    let gitTagsResult = await execCmd("git", "tag", "-l", PACKAGE_NAME ? `${PACKAGE_NAME}@*` : "", PACKAGE_NAME && INCLUDE_NIGHTLY ? "v0.0.0-nightly-*" : "", "--sort", "-creatordate", "--format", "%(refname:strip=2)");
    ...
    let isStable = semver_1.default.prerelease(latest.clean) === null;
    ...
    debug(JSON.stringify({ latest, previous, isPreRelease, isStable, isNightly }));
    return { previous, latest, isStable };
}
async function getCommits(from, to) {
    let gitCommitsResult = await execCmd("git", "log", "--pretty=format:%H", `${from.raw}...${to.raw}`, DIRECTORY_TO_CHECK);
...
async function commentOnPrAndLinkedIssues(pr, latest, isStable) {
    ...
    if (DRY_RUN) {
        ...
        // Comment on PR
        promises.push(execCmd("gh", "pr", "comment", String(pr.number), "--body", prComment));
        // Remove PR labels for stable releases
        if (PR_LABELS_TO_REMOVE && isStable) {
            promises.push(execCmd("gh", "pr", "edit", String(pr.number), "--remove-label", PR_LABELS_TO_REMOVE));
        }
    }
...
async function commentOnIssue(issue, latest, isStable) {
    ...
    if (DRY_RUN) {
        ...
        if (ISSUE_LABELS_TO_REMOVE && isStable) {
            debug(`[dry-run] would remove label "${ISSUE_LABELS_TO_REMOVE}" from issue #${issue}`);
        }
    }
    else {
        // Comment on linked issue
        await execCmd("gh", "issue", "comment", String(issue), "--body", issueComment);
        // Close linked issue
        if (shouldClose) {
            await execCmd("gh", "issue", "close", String(issue));
        }
        else {
            debug(`Skipping close of issue #${issue} due to "${ISSUE_LABELS_TO_KEEP_OPEN}" label`);
        }
        // Remove labels from linked issue
        if (ISSUE_LABELS_TO_REMOVE && isStable) {
            await execCmd("gh", "issue", "edit", String(issue), "--remove-label", ISSUE_LABELS_TO_REMOVE);
        }
    }
}
function findPreviousStableRelease(tag, gitTags) {
    let stableTags = getStableTags(gitTags);
    let expectedMajor = semver_1.default.major(tag.clean);
...
function getStableTags(tags) {
    return tags.filter((tag) => {
        return semver_1.default.prerelease(tag.clean) === null;
    });
}
...
let pullRequestResultSchema = zod_1.z.object({
...
async function findMergedPRs(commits) {
    ...
    let result = await Promise.all(commits.map(async (commit) => {
        let prResult = await execCmd("gh", "pr", "list", "--search", commit, "--state", "merged", "--json", "number,title,url,body");
        if (prResult.stderr) {
        ...
        debug(JSON.stringify({ pr: pr.number, linkedIssues, issuesClosedViaBody }));
    ...
    return result.filter((pr) => pr != undefined);
}
let referencedIssueResultSchema = zod_1.z.object({
    data: zod_1.z.object({
        resource: zod_1.z.object({
            closingIssuesReferences: zod_1.z.object({
                nodes: zod_1.z.array(zod_1.z.object({ number: zod_1.z.number() })),
...
async function getIssuesLinkedToPullRequest(prHtmlUrl) {
  ...
  `;
    let result = await execCmd("gh", "api", "graphql", "--paginate", "--field", `prHtmlUrl=${prHtmlUrl}`, "--raw-field", `query=${(0, trim_newlines_1.trimNewlines)(query)}`);
    if (result.stderr) {
    ...
    return valid.data.data.resource.closingIssuesReferences.nodes.map((node) => node.number);
}
let issueLabelsSchema = zod_1.z.object({
    data: zod_1.z.object({
        repository: zod_1.z.object({
            issue: zod_1.z.object({
                number: zod_1.z.number(),
                title: zod_1.z.string(),
                url: zod_1.z.string(),
...
async function getIssueLabels(number) {
    let gql = String.raw;
    let query = gql `
  ...
  `;
    ...
    let result = await execCmd("gh", "api", "graphql", "--field", `owner=${owner}`, "--field", `repo=${repo}`, "--field", `number=${number}`, "--raw-field", `query=${(0, trim_newlines_1.trimNewlines)(query)}`);
    if (result.stderr) {
        core.error(result.stderr);
        throw new Error(result.stderr);
    }
...
async function execCmd(command, ..._args) {
    let args = _args.filter((arg) => arg.length > 0);
    debug(`> ${command} ${args.join(" ")}`);
    let result = await (0, execa_1.execa)(command, args);
    return result;
}
main().then(() => {
    process.exit(0);
...
var signal_exit = __nccwpck_require__(3289);
...
var get_stream = __nccwpck_require__(1049);
...
/* harmony export */   "trimNewlines": () => (/* binding */ trimNewlines),
/* harmony export */   "trimNewlinesEnd": () => (/* binding */ trimNewlinesEnd),
/* harmony export */   "trimNewlinesStart": () => (/* binding */ trimNewlinesStart)
/* harmony export */ });
